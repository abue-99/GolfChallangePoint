...
6. **Journey assignment queue**: `JourneyTemplateAssignment` stores direct player/team journey assignments as persisted queue records, and auth-protected write/read access depends on the web proxy layer converting cookie auth into bearer Authorization before requests hit journey endpoints.
