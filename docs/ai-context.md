...
- **Learning progress summaries**: coach-facing `/teams/club-players` and `/users/me/players` responses now include `learningProgress` with journey and lesson lifecycle counts plus 90-day `recentCompletions` metadata for compact avatar cards and active-player dashboard views; shared coach player cards render a two-line `Journeys`/`Lessons` summary with compact lifecycle color bars (showing only non-zero states at fixed quarter-width segments), while popup detail views render icon-free four-slot lifecycle bars with fixed segment widths and use persisted assignment `completedAt` timestamps to control recent-completion visibility.
- **Teams workspace layout**: `/teams` and `/coach/teams` share a mobile-first workspace with collapsible `Teams` and `Players` sections persisted in `localStorage`; both sections render as separately scrollable horizontal card rails constrained to the visible viewport, while `Journey Library` sits above `Lesson Library` in a visually separated sidebar on desktop and stacks below the main content on mobile. Team cards use a narrower compact layout with up to five smaller member avatars and show assigned player/journey counts instead of team-level pending labels.
- **Coach player detail workspace**: `/coach/players/[playerId]` uses `DevelopmentPlanManager` as a mobile-first assignment hub with inline lesson/journey template assignment, a consolidated lessons list above journeys, lifecycle filters/search, and journey origin badges (`Template`/`Individual`/`Team`).
- **Active player detection**: coach dashboard active-player sections now treat a player as active only when journey lifecycle counts contain `PENDING`, `ACCEPTED`, or `ACTIVE`.
