...
- `recentCompletions`: 90-day completed totals used only by compact avatar/dashboard cards that render `J`/`L` non-zero status chips; popup views still show full historical `COMPLETED` totals
